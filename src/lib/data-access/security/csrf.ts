...
type CSRFToken = Database['public']['Tables']['csrf_tokens']['Row'];
...
export async function validateCSRFToken(token: string): Promise<boolean> {
  ...
  if (token !== cookieToken) {
    console.warn('CSRF token mismatch: form token does not match cookie');
    return false;
  }
  ...
  if (!sessionId) {
    console.warn('CSRF validation failed: no session ID');
    return false;
  }
  ...
  if (error || !data) {
    console.warn('CSRF token validation failed:', error || 'Token not found');
    return false;
  }
...
export async function revokeUserTokens(userId: string): Promise<void> {
  ...
  if (error) {
    console.error('Failed to revoke user CSRF tokens:', error);
  }
}
