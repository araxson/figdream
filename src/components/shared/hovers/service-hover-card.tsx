...
import { HoverCard, HoverCardContent, HoverCardTrigger, Badge, Separator } from "@/components/ui"
...
export function ServiceHoverCard({ service, children, side = "right" }: ServiceHoverCardProps) {
  ...
  const getPopularityBadge = (rank?: number) => {
    ...
    if (rank <= 3) {
      return <Badge variant="default" className="text-xs">Most Popular</Badge>
    } else if (rank <= 10) {
