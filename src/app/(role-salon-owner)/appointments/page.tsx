...
import { AppointmentsFilter } from '@/components/salon-owner/appointments/appointments-filter'
import { StaffHoverCard, CustomerHoverCard, ServiceHoverCard } from "@/components/shared/hovers"
...
export default async function AppointmentsPage() {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      pending: { label: "Pending", variant: "outline" as const, icon: AlertCircle, tooltip: "Appointment pending confirmation" },
      confirmed: { label: "Confirmed", variant: "default" as const, icon: CheckCircle, tooltip: "Appointment confirmed and scheduled" },
      in_progress: { label: "In Progress", variant: "secondary" as const, icon: Clock, tooltip: "Service currently in progress" },
      completed: { label: "Completed", variant: "default" as const, icon: CheckCircle, tooltip: "Appointment successfully completed" },
      cancelled: { label: "Cancelled", variant: "destructive" as const, icon: XCircle, tooltip: "Appointment has been cancelled" },
      no_show: { label: "No Show", variant: "destructive" as const, icon: XCircle, tooltip: "Customer did not show up" }
    }
                ...
                {appointments.map((appointment) => {
                  ...
                  return (
                    <TableRow key={appointment.id}>
                      ...
                      <TableCell>
                        {staff ? (
                          <StaffHoverCard staff={{
                            ...staff,
                            profiles: staff.profiles || null,
                            display_name: staff.profiles?.full_name || staff.title || undefined
                          }}>
                            <button className="text-left hover:text-primary transition-colors">
                              {staff.profiles?.full_name || "Unassigned"}
                            </button>
                          </StaffHoverCard>
                        ) : (
                          "Unassigned"
                        ...
                        <div className="space-y-1">
                          {services.slice(0, 2).map((service, idx) => (
                            <div key={service.id} className="text-sm">
                              {service.services ? (
                                <ServiceHoverCard service={service.services}>
                                  <button className="text-left hover:text-primary transition-colors">
