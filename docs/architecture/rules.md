...
- **CRITICAL:** Never use `raw_app_meta_data` in RLS policies - it can be modified by authenticated users
...
- **CRITICAL:** Always use `raw_app_meta_data` instead of `raw_app_meta_data` in RLS policies
