...
function calculateTrendFactor(bookings: any[], daysAhead: number): number {
...
function calculateConfidence(patterns: any, dayOfWeek: number): number {
  // Confidence based on data consistency
  return Math.min(95, 70 + Math.random() * 25)
}

function calculateOverallConfidence(patterns: any): number {
  return 75 + Math.random() * 15
}

function generateDemandInsights(forecast: any[], patterns: any): string[] {
  const insights = []
...
function calculateChurnRisk(customer: any, appointments: any[]): any {
  const now = new Date()
...
function generateChurnInsights(risks: any[]): string[] {
  const insights = []
...
function aggregateMonthlyRevenue(transactions: any[]): any[] {
...
function calculateRevenueTrend(monthlyRevenue: any[]): any {
...
function calculateProjectionConfidence(trend: any, monthsAhead: number): number {
  const baseConfidence = trend.confidence
  const decay = monthsAhead * 5
  return Math.max(50, baseConfidence - decay)
}

function generateRevenueInsights(projections: any[], trend: any, historical: any[]): string[] {
  const insights = []
...
function generateStaffingInsights(recommendations: any[]): string[] {
  const insights = []
...
function calculateStaffingEfficiency(recommendations: any[]): number {
