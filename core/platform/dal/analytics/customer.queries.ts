...
export async function getCustomerMetrics(
  ...
  if (salonId) {
    ...
    uniqueCustomers = new Set(appointments?.map(a => a.customer_id).filter((id): id is string => Boolean(id)) || []);
  } else {
    const { data: profiles } = await supabase
      .from("profiles")
      .select("id");

    uniqueCustomers = new Set(profiles?.map(p => p.id).filter((id): id is string => Boolean(id)) || []);
  }
