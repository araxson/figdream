...
import { getLoyaltyStats, getTopLoyaltyCustomers } from '@/lib/data-access/loyalty-admin'
...
import CustomerPointsTable from './components/customer-points-table'
...
export default async function LoyaltyAdminPage() {
  ...
  // Get loyalty stats and top customers
  const [loyaltyData, topCustomers] = await Promise.all([
    getLoyaltyStats(userRole.salon_id),
    getTopLoyaltyCustomers(userRole.salon_id, 5)
  ])

  const { program, stats } = loyaltyData
                    ...
                    <div key={customer.id} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <Badge variant="outline" className="text-xs">
                          <span className={`h-2 w-2 rounded-full mr-1 ${getTierColor(customer.tier)}`} />
                          {customer.tier}
                        </Badge>
                      </div>
                    </div>
