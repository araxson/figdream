...
- **CRITICAL:** Fix ALL ESLint errors before committing any code - zero tolerance policy
- **CRITICAL:** Run `npm run lint` before every commit and fix ALL errors
- **CRITICAL:** Never commit code with ESLint errors or warnings
- **CRITICAL:** ESLint errors block all deployments and merges
...
- **CRITICAL:** Replace all `any` types with proper TypeScript types
- **CRITICAL:** Remove all `@ts-nocheck` comments and fix underlying issues
- **CRITICAL:** Fix all `@typescript-eslint/no-unsafe-function-type` errors
- **CRITICAL:** Ensure all function parameters and return types are properly typed
...
- **CRITICAL:** Do NOT rely on middleware for authentication (CVE-2025-29927) - use Data Access Layer instead
...
- **AVOID:** Using `user_metadata` in RLS policies (use `app_metadata` instead)
...
- **CRITICAL:** Use `@` path aliases for ALL cross-directory imports throughout the application code
- **CRITICAL:** Configure and maintain comprehensive path aliases in `tsconfig.json` for consistent imports
...
- **CRITICAL:** Never use `user_metadata` in RLS policies - it can be modified by authenticated users
...
- **AVOID:** Using user-modifiable data (`user_metadata`) in security-critical policies
...
- **FOLLOW:** Use `raw_app_meta_data` instead of `user_metadata` for authorization data
...
- **AVOID:** Using `user_metadata` in RLS policies (can be modified by users)
...
- **CRITICAL:** Data Access Layer is now the PRIMARY recommended approach for authentication (middleware is no longer safe)
...
- **CRITICAL:** SearchParams should never be used for authorization (e.g., never trust `?isAdmin=true`)
...
- **CRITICAL:** Middleware is NO LONGER safe for authentication (CVE-2025-29927)
...
- **CRITICAL:** Middleware is no longer safe for authentication (CVE-2025-29927)
- **CRITICAL:** Data Access Layer (DAL) is now the mandatory approach for authentication
- **CRITICAL:** Always use `raw_app_meta_data` instead of `user_metadata` in RLS policies
- **CRITICAL:** SearchParams must never be used for authorization decisions
...
- **FOLLOW:** Propagate tenant headers (`x-tenant-id`, `x-tenant-slug`, `x-tenant-status`) downstream as needed.
- **FOLLOW:** Enforce route protection for tenant areas such as `/dashboard`, `/staff`, `/clients`, `/appointments`, `/services`.
...
- **CRITICAL:** NEVER use mock data, fake data, or hardcoded sample data anywhere in the application
- **CRITICAL:** ALL data MUST come from Supabase database or external APIs - no exceptions
- **CRITICAL:** Remove any existing mock data, fake data, or hardcoded sample data immediately
...
- **FOLLOW:** Keep files under 500 lines for maintainability; break down large files into smaller, modular components.
- **FOLLOW:** Limit each folder to maximum 10 files and maximum 10 subfolders for optimal organization.
...
- **AVOID:** Creating folders with more than 10 files or 10 subfolders.
- **AVOID:** Mixing naming conventions within the same context.
- **AVOID:** Overly long or unnecessarily descriptive file names.

### **39. Import/Export Optimization & Re-export Management**

- **FOLLOW:** Identify and fix unnecessary re-export files (single-line import/export files that just pass through imports).
- **FOLLOW:** Consolidate re-exports into proper barrel exports when multiple related exports exist.
- **FOLLOW:** Remove re-export files entirely and update imports to reference original sources directly.
- **FOLLOW:** Optimize import/export patterns by removing unnecessary intermediate re-export layers.
- **FOLLOW:** Update all import statements immediately after file moves and re-export cleanup.
- **FOLLOW:** Use barrel exports only when they provide clear organizational benefits.
- **FOLLOW:** When moving files, update ALL import statements to point to the new location directly.
- **FOLLOW:** Go to each file that imports the moved component and change the import path.
- **AVOID:** Creating single-line re-export files that just pass through imports from different locations.
- **AVOID:** Maintaining unnecessary intermediate re-export layers.
- **AVOID:** Re-exporting from multiple unrelated locations in a single file.
- **AVOID:** Creating new folders or files just to re-export moved components.
- **AVOID:** Using re-export files as a shortcut instead of updating import paths directly.
- **AVOID:** Creating intermediate re-export layers when you can simply update the import statement.
