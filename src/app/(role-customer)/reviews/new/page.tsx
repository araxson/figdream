  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createReview({
        ...
        service_id: selectedAppointment.appointment_services?.[0]?.services?.id || undefined,
        staff_id: selectedAppointment.staff_id || undefined,
        booking_id: selectedAppointment.id,
        customer_id: selectedAppointment.customer_id, // Add required customer_id
        rating,
        title: title.trim() || undefined,
        content: content.trim(),
        photos: photos.length > 0 ? photos.map(url => ({ url })) : undefined,
        is_anonymous: false, // Add required is_anonymous field
        source: 'website' as const, // Add required source field
        verified_purchase: true, // Add required verified_purchase field
      });
                    ...
                    <SelectItem key={apt.id} value={apt.id}>
                      <div className="flex flex-col">
                        <span className="font-medium">
                          {apt.salons?.name} - {apt.appointment_services?.[0]?.services?.name || 'Service'}
                        </span>
                        <span className="text-sm text-muted-foreground">
                          {new Date(apt.appointment_date).toLocaleDateString()} with {apt.staff_profiles?.profiles?.full_name || 'Staff'}
                        </span>
                      </div>
                    </SelectItem>
                  ))}
