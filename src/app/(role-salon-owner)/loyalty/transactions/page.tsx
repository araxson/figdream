...
export default async function LoyaltyTransactionsPage({
  ...
  const { transactions, total } = await getLoyaltyTransactions(userRole.salon_id, {
    customerId: searchParamsResolved.customer,
    type: searchParamsResolved.type,
    startDate: searchParamsResolved.start,
    endDate: searchParamsResolved.end,
    limit,
    offset
  })
                ...
                {transactions.length === 0 ? (
                  ...
                  transactions.map((transaction) => (
                    <TableRow key={transaction.id}>
                      ...
                      <TableCell>
                        <Badge variant={getTransactionBadgeVariant(transaction.type)} className="gap-1">
