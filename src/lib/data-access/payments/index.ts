...
import { canAccessAppointment } from '@/lib/data-access/auth/permissions'
  ...
  try {
    ...
    if (user.role === 'customer') {
      ...
      filteredPayments = filteredPayments.filter(p => 
        (p.appointments as Record<string, unknown>)?.customer_id === user.user_id
      )
      ...
      filteredPayments = filteredPayments.filter(p => 
        (p.appointments as Record<string, { salons?: { owner_id?: string } }>)?.locations?.salons?.owner_id === user.user_id
      )
    }
    ...
    const cleanPayments = filteredPayments.map(p => {
      const { appointments: _appointments, ...payment } = p
...
export async function getLocationPayments(
  ...
  try {
    ...
    const paymentsWithAppointment = (payments || []).map(p => ({
      ...p,
      appointment: p.appointments as unknown as Appointment
