...
import { Card, CardContent, CardDescription, CardHeader, CardTitle, Badge, Tabs, TabsContent, TabsList, TabsTrigger, Table, TableBody, TableCell, TableHead, TableHeader, TableRow, Alert, AlertDescription, Separator } from '@/components/ui';
import { DollarSign, Calendar, CreditCard, Receipt, AlertCircle, TrendingUp, TrendingDown, Clock } from 'lucide-react';
...
export default async function CustomerPaymentsPage() {
  ...
  // Calculate payment statistics based on appointment status
  // Completed appointments are considered paid
  const paidAppointments = appointments?.filter(apt => apt.status === 'completed') || [];
  const unpaidAppointments = appointments?.filter(apt => 
    apt.status === 'confirmed' || apt.status === 'pending'
  ) || [];
  const totalPaid = paidAppointments.reduce((sum, apt) => sum + (apt.total_amount || 0), 0);
  const totalPending = unpaidAppointments.reduce((sum, apt) => sum + (apt.total_amount || 0), 0);
  ...
  const recentPayments = paidAppointments.filter(apt => 
    new Date(apt.completed_at || apt.appointment_date) >= sixMonthsAgo
  );
  
  const monthlySpending = recentPayments.reduce((acc, apt) => {
    const date = new Date(apt.completed_at || apt.appointment_date);
    const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
    acc[monthKey] = (acc[monthKey] || 0) + (apt.total_amount || 0);
    return acc;
                  ...
                  <TableBody>
                    {appointments.map((apt) => (
                      <TableRow key={apt.id}>
                        ...
                        <TableCell>{apt.appointment_services?.[0]?.services?.name || 'Multiple services'}</TableCell>
                        <TableCell>
                          {apt.staff_profiles?.profiles?.full_name || 'N/A'}
                        </TableCell>
                        <TableCell className="font-medium">
                          {formatCurrency(apt.total_amount || 0)}
                        </TableCell>
                        <TableCell>
          ...
          <CardContent>
            ...
            <div className="text-center">
              <p className="text-sm text-muted-foreground">
                Average spending: <span className="font-medium">{formatCurrency(averageMonthlySpending)}</span> per month
              </p>
            </div>
          </CardContent>
        </Card>
