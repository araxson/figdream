...
import { getCustomerByUserId, getCustomerAppointments, getCustomerLoyaltyPoints, getCustomerAnalytics } from '@/lib/data-access/customers';
    ...
    <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <Badge variant="outline">{analytics?.average_days_between_visits ? `Every ${analytics.average_days_between_visits} days` : 'New Customer'}</Badge>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Average Spend</span>
                <span className="font-semibold">${((analytics?.lifetime_revenue || 0) / Math.max(analytics?.lifetime_appointments || 1, 1)).toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Favorite Service</span>
                <span className="text-sm font-medium">{analytics?.preferred_services ? 'View in profile' : 'None yet'}</span>
              </div>
              {analytics?.days_since_last_visit !== null && analytics?.days_since_last_visit !== undefined && (
                <div className="flex justify-between items-center">
            ...
            <CardContent className="space-y-4">
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-semibold">${(analytics?.lifetime_revenue || 0).toFixed(2)}</span>
                </div>
                <Progress value={Math.min((analytics?.lifetime_revenue || 0) / 1000 * 100, 100)} className="h-2" />
                <p className="text-xs text-muted-foreground mt-1">
                  ${(1000 - (analytics?.lifetime_revenue || 0)).toFixed(2)} to VIP status
                </p>
              </div>
              <div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-muted-foreground">Total Visits</span>
                  <span className="font-semibold">{analytics?.lifetime_appointments || 0}</span>
                </div>
                <Progress value={Math.min((analytics?.lifetime_appointments || 0) / 10 * 100, 100)} className="h-2" />
                <p className="text-xs text-muted-foreground mt-1">
                  {Math.max(10 - (analytics?.lifetime_appointments || 0), 0)} visits to next reward
                </p>
              </div>
              {analytics?.retention_rate !== null && (
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm text-muted-foreground">Loyalty Score</span>
                    <Badge variant={(analytics?.retention_rate || 0) > 70 ? "default" : "secondary"}>
                      {Math.round(analytics?.retention_rate || 0)}%
                    </Badge>
                  </div>
                </div>
              )}
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          ...
          <Button asChild variant="outline">
            <Link href="/role-customer/profile">
          ...
          <CardContent>
            ...
            {upcomingAppointments.length > 3 && (
              <div className="mt-4 text-center">
                <Link
                  href="/role-customer/appointments"
                  className="text-sm text-blue-600 hover:underline"
                >
